...
function print_help {
    echo "$0 <name> <step,+> <processer> <genomeVersion> <readsFile+> <ensGene|refGene>"
}
function join_by {
...
function mapping {
    if [[ ! -e 1_mapping/${name}.bam ]]; then
        ...
        hisat2 --summary-file 1_mapping/Mapping_${name}.log --new-summary -p ${processer} --mm -x ~/source/bySpecies/${genomeVersion}/hisat2/${genomeVersion}_main -U ${mapping_input_file} | samtools view -@ $((${processer}-1)) -bS > 1_mapping/${name}.bam && \
        rm ${filteredReads[@]}
    fi
}
